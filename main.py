...
PLAYER_RPG_IMAGE = pygame.image.load("Assets/Images/player with rpg.png").convert_alpha()
...
ENEMY_PLAYER_RPG_IMAGE = pygame.image.load("Assets/Images/enemy player with rpg.png").convert_alpha()

BULLET_IMAGE = pygame.image.load("Assets/Images/bullet.png").convert_alpha()
ROCKET_IMAGE = pygame.image.load("Assets/Images/rpg.png").convert_alpha()
...
class Player(Entity):
    ...
    def draw(self, type):
        if type == "enemy":
            if self.current_weapon == "pistol":
                super().draw(ENEMY_PLAYER_PISTOL_IMAGE)
            elif self.current_weapon == "shotgun":
                super().draw(ENEMY_PLAYER_SHOTGUN_IMAGE)
            elif self.current_weapon == "auto_rifle":
                super().draw(ENEMY_PLAYER_AUTO_RIFLE_IMAGE)
            elif self.current_weapon == "sniper":
                super().draw(ENEMY_PLAYER_SNIPER_IMAGE)
            elif self.current_weapon == "rpg":
                super().draw(ENEMY_PLAYER_RPG_IMAGE)
            self.display_health()
        else:
            if self.current_weapon == "pistol":
                super().draw(PLAYER_PISTOL_IMAGE)
            elif self.current_weapon == "shotgun":
                super().draw(PLAYER_SHOTGUN_IMAGE)
            elif self.current_weapon == "auto_rifle":
                super().draw(PLAYER_AUTO_RIFLE_IMAGE)
            elif self.current_weapon == "sniper":
    ...
    def update(self, walls, bullets, enemies, explosions):
        self.move(walls)
        self.rotate()
        self.check_if_hit(bullets, enemies, explosions)
    ...
    def check_if_hit(self, bullets, enemies, explosions):
    ...
    def shoot_rpg(self, new_bullets):
        rocket = Rocket((self.x_pos, self.y_pos))
        new_bullets.append(rocket)
...
class Explosion(Entity):
    def __init__(self, position, size, time_left, damage):
        super().__init__(position, size, size)
...
class Enemy(Entity):
    ...
    def update(self, player_pos, bullets, explosions):
        self.check_damage(bullets, explosions)
        self.move(player_pos[0], player_pos[1])

    def draw(self, image):
        super().draw(ENEMY_IMAGE)
        self.display_health()

    def check_damage(self, bullets, explosions):
        for bullet in bullets:
            if bullet.rect.colliderect(self.rect):
                bullets.remove(bullet)
                self.health -= bullet.damage
...
def redraw_window(player, enemy_player, walls, bullets, enemies, boxes, explosions, background_image, ui):
    ...
    if enemy_player:
        enemy_player.draw("enemy")

    if player:
        player.draw("main")

    for explosion in explosions:
            explosion.draw(EXPLOSION_IMAGE)
...
def map_selection_screen():
    ...
    while True:
        ...
        redraw_window(None, [], [], [],  [], [], [], BACKGROUND_IMAGE, ui)
...
def difficulty_selection_screen():
    ...
    ui = [select_difficulty_text] + buttons
...
def multiplayer_menu():
    ...
    ui = text_boxes + buttons + input_boxes
